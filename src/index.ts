...
// Mount each snap app at its path
app.get("/snapunks/placeholder", (c) => snapunks.fetch(c.req.raw));
app.all("/snapscored", (c) => snapscored.fetch(c.req.raw));
app.all("/snapscored/*", (c) => snapscored.fetch(c.req.raw));
app.all("/snapunks", (c) => snapunks.fetch(c.req.raw));
app.all("/snapunks/*", (c) => snapunks.fetch(c.req.raw));
...
<body>
  ...
  <div class="grid">
    <a class="card" href="/snapscored"><span class="icon">🏅</span><div class="info"><h2>SnapScored</h2><p>Check your Neynar score — Diamond or Newcomer?</p></div></a>
    <a class="card" href="/snapunks"><span class="icon">👾</span><div class="info"><h2>SnaPunks</h2><p>Claim your unique generative pixel punk art</p></div></a>
  </div>
